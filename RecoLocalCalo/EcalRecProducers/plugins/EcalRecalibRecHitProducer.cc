...
EcalRecalibRecHitProducer::EcalRecalibRecHitProducer(const edm::ParameterSet& ps) :
    ...
    doTimeCalib_(               ps.getParameter<bool>("doTimeCalib") ),
    ...
    doTimeCalibInverse_(        ps.getParameter<bool>("doTimeCalib") )
{
...
void EcalRecalibRecHitProducer::produce(edm::StreamID sid, edm::Event& evt, const edm::EventSetup& es) const
{
    ...
    if (EBRecHits) {
        ...
        for(EBRecHitCollection::const_iterator it  = EBRecHits->begin(); it != EBRecHits->end(); ++it) {
            ...
            EcalTimeCalibConstant timecalib = 0;
            if (doTimeCalib_) {
                // The time calibration comprises two terms:
                //   1) global partition (EB/EE) offset
                //   2) channel to channel intercalibration
                // 1) is assumed to be unchenged here, only 2) is adjusted.
                // In order to change the time calibration from c1 to c2 one has to 
                // provide a tag that contains c1-c2.
                const EcalTimeCalibConstantMap & itimeMap = itime->getMap();  
                EcalTimeCalibConstantMap::const_iterator itime = itimeMap.find(it->id());
            ...
            if (doTimeCalibInverse_) {
                timecalib = -timecalib;
            }

            // make the rechit and put it in the output collection            
            EcalRecHit aHit( *it );
            aHit.setEnergy(it->energy() * agc_eb * icalconst * lasercalib);
            aHit.setTime(it->time() + timecalib);
    ...
    if (EERecHits)
    {
        ...
        for(EERecHitCollection::const_iterator it  = EERecHits->begin();
            ...
            EcalTimeCalibConstant timecalib = 0;
            if (doTimeCalib_) {
                // NOTE: see comment above in EB section
                const EcalTimeCalibConstantMap & itimeMap = itime->getMap();  
                EcalTimeCalibConstantMap::const_iterator itime = itimeMap.find(it->id());
                if( itime!=itimeMap.end() ) {
                    timecalib = (*itime);
            ...
            // make the rechit and put it in the output collection            
            EcalRecHit aHit( *it );
            aHit.setEnergy(it->energy() * agc_ee * icalconst * lasercalib);
            aHit.setTime(it->time() + timecalib);
